...
private:
	...
	bool gamePaused;
	int linesToClear;
	std::vector<int> linesToRemove;  // Сохраняем индексы линий для удаления
	double animationTimer;
	double gameTimer;
	bool fastDrop;
	double timeSinceLastDrop;

	// Новая логика: уровни и счетчик линий
	int level = 1;
	...
	// Параметры падения
	double baseDropInterval = 0.8;   // базовый интервал на уровне 1
	double fastDropInterval = 0.05;  // быстрый софт-дроп
